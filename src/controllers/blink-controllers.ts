...
import { getReferralById } from '@/services/referral-services';
...
export const handelGetRefBlinkMetaData = createHandler(widgetIdParamsSchema, async (req, res) => {
  const { id } = req.params;
    ...
    const metadata = {
      ...
      links: {
        actions: [
          {
            label: '$50',
            href: `${process.env.API_BASE_URL}/blink/action/ref/buy/${id}/50`,
          },
          {
            label: '$100',
            href: `${process.env.API_BASE_URL}/blink/action/ref/buy/${id}/100`,
          },
          {
            label: '$200',
            href: `${process.env.API_BASE_URL}/blink/action/ref/buy/${id}/200`,
          },
          {
            label: '$500',
            href: `${process.env.API_BASE_URL}/blink/action/ref/buy/${id}/500`,
          },
          {
            href: `${process.env.API_BASE_URL}/blink/action/ref/buy/${id}/{amount}`,
            label: `Buy ${widget.token.symbol}`,
            parameters: [
              {
                name: 'amount',
...
export const handelGetRefPaymentTransaction = createHandler(z.object({
  params: z.object({ id: z.string(), amount: z.string() }),
  body: z.object({ account: z.string() }),
}), async (req, res) => {
  const { id, amount } = req.params;
